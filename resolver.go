...
// NewPolarisResolverV2 creates a polaris based resolver.
func NewPolarisResolverV2(o Options, configFile ...string) (Resolver, error) {
	sdkCtx, err := GetPolarisConfig(configFile...)
	if err != nil {
		return nil, err
	}
...
func (pr *polarisResolver) Target(ctx context.Context, target rpcinfo.EndpointInfo) (description string) {
	...
	dstNamespace := pr.o.DstNamespace
...
func (pr *polarisResolver) Watcher(ctx context.Context, desc string) (discovery.Change, error) {
	...
	if nil != instances {
		for _, instance := range instances {
			...
			eps = append(eps, ChangePolarisInstanceToKitexV2(instance, pr.o))
		}
	}
